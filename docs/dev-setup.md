...
# API を更新する場合
az acr build --registry acrtvmvp73bb --image tvmvp-api:latest --file backend/Dockerfile .
az containerapp update -g rg-001-gen12 -n ca-tvmvp-api \
  --image acrtvmvp73bb.azurecr.io/tvmvp-api:latest

# worker を更新する場合（ffmpeg＋BGM 入りイメージ。BGM は worker/assets/bgm/ を同梱）
az acr build --registry acrtvmvp73bb --image tvmvp-worker:latest --file worker/Dockerfile .
# tag が latest のままだと新リビジョンが立たないことがあるため、digest 指定で更新すると確実:
DIGEST=$(az acr manifest list-metadata -r acrtvmvp73bb -n tvmvp-worker \
  --query "sort_by([?tags], &lastUpdateTime)[-1].digest" -o tsv)
az containerapp update -g rg-001-gen12 -n ca-tvmvp-worker \
  --image acrtvmvp73bb.azurecr.io/tvmvp-worker@$DIGEST
```
...
# API の URL を埋め込んで静的ビルド（末尾スラッシュ無しの API URL）
