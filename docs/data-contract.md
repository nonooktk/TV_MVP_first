...
| `trigger_source` | 発火元（声トリガーの両側化・2026-07-10 追加）。`"elder"`（高齢者側リモート音声）／`"family"`（家族側ローカルマイク・第2系統）。省略（過去データ）は `elder` 扱い |
| `stream` | 写真の取得元カメラ（両側連写・Phase 2・2026-07-10 追加）。`"elder"`（高齢者側リモート映像）／`"family"`（家族側ローカルカメラ・孫が映る側）。1発火で両ストリームの写真が生成される。省略（過去データ）は `elder` 扱い。音声スニペットには付与しない（音声は高齢者側のみ） |
| `lookback` | look-back（発火前バッファ由来）か否か（bool） |

> `trigger_reason` の `centroid`（スペクトル重心トリガー・改良2）: 音圧が変わらなくても
> 声色（笑い声・高い声・興奮）で重心が基準比 +30% を 200ms 持続したときの発火。
> `rms`/`stt` と**共有のクールダウン（8秒。2026-07-07 に 4秒→8秒）**が適用される。worker は
> reason で分岐しない（stage1 のスコアリングは per-photo の rms_rise / face_score を使う。
> ラベリング文脈では `声色の変化` として集計される）。`spectral_centroid` / `centroid_rise_ratio`
> は発火要因にかかわらず全写真へ付与する（発火時点の重心サンプル）。
>
> `trigger_source`（声トリガーの両側化・2026-07-10 追加）: 家族側ローカルマイク音声にも
> 第2の検知系統（family lane）を追加し、家族側の歓声・盛り上がりでも発火できるようにした。
> family lane は elder（高齢者側リモート音声）とは独立の baseline・発話累計・ノイズフロア
> 推定・ノイズゲート・リアームを持つが、**クールダウンは elder/family 全系統で共有**（8秒。
> どちらが発火しても他方も含めて次の8秒は再発火しない）。STT は elder のみ（family lane には
> 付けない）。写真連写は現状どおり高齢者側の video リングから行う（両側連写は次フェーズ）。
> worker はラベリング文脈（`call_context.py`）でのみ `trigger_source` を軽く参照する
> （`trigger_source == "family"` の写真が1枚でもあれば「家族側の歓声」の文脈語を1行足す。
> stage1 のスコアリングロジックは変更なし）。
>
> `trigger_reason` の `face`（顔トリガー・顔検知の家族側化・Phase 2・2026-07-10）: MediaPipe
> 表情検知を高齢者側リモート映像 → **家族側ローカルカメラ**（孫が映る側）へ切り替え、家族側の
> face_score が絶対閾値 +0.7 を 300ms 持続したときに `trigger_source="family"` で発火する。
> `rms`/`stt`/`centroid` と**共有のクールダウン（8秒）**に参加する。worker の stage1 は
> reason で分岐しない（ラベリング文脈で `表情` として集計し、`face` があれば「孫の笑顔・変顔」の
> 文脈語を1行足す）。
>
> `stream`（両側連写・Phase 2・2026-07-10）: どのトリガーでも発火時に高齢者側リング＋家族側
> リングの両方から10枚ずつ（計20枚）連写し、各写真に取得元カメラ `stream`（`"elder"`／
> `"family"`）を付ける。家族側写真には家族側 facePipeline のコマ別 `face_score`、高齢者側写真は
> `face_score=0`（顔検知は家族側のみ＝負荷対策）。worker の stage1 は**無表情ゲート・音圧
> フォールバック・rms_rise 正規化をストリーム別に適用**する（高齢者側 face=0 が家族側の表情信号を
> 理由に全滅して候補が家族側に独占されるのを防ぐ＝両ストリームが候補に混在して返る）。
> 音声スニペットは高齢者側のみで `stream` を付与しない。
