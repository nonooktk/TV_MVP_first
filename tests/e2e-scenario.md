...
3. 候補サムネイルを5枚タップして選択し、「これで確定」を押す
   ...
   - （2026-07-06 追加）**rank 1〜5 の候補カードには「おすすめ」バッジ**（左下・オレンジの
     小ラベル）が表示される。**「おすすめの5枚を選ぶ」ボタン**を押すと rank 1〜5 が
     一括で選択状態になる（その後の入替は従来どおりタップで可能。確定は「これで確定」）。
...
- 選択UI: 「自動確定まで 残り M分SS秒」のカウントダウン表示。5枚選ぶまで
  ...
  rank 1〜5 に「おすすめ」バッジが付き、「おすすめの5枚を選ぶ」で一括選択できる
  （2026-07-06 追加。一括選択後の入替・確定の挙動は従来どおり）。
