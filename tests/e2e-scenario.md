...
> **ステータス注記（2026-07-04 更新・M2 完了）**: 本書は RFP 12章（検収条件）の判定手順書である。
> **M1（通話=Agora実接続）・M2（検知コア②＝感情検知・自動キャプチャ＋通話後同期）とも実装済み**。
> 通話（ステップ2〜3）・自動キャプチャ（ステップ3）・終了と同期（ステップ4）はいずれも
> 正式手順で判定できる。検知の発火は実発話（役者が声量を上げる）で起こせるほか、
...
> （`docs/dev-setup.md` §12）。**STT（発話キーワード）は削減ラダー②により未接続**
> （Azure Speech 取得後に追加）。音声 look-back はチャンク結合方式（後述）。
> 疑似E2E版の通し実施は 2026-07-03、M1/M2 の自動E2Eは 2026-07-04 に完了。
...
| コア②: 初期値パラメータで発火 → 連写10枚＋音声スニペットがIndexedDBに保存される | 3 | ✓ 判定可（M2済み。フルチェーンE2Eで連写10枚＋音声スニペット1件を IndexedDB に確認。音声 look-back はチャンク結合方式=先頭ヘッダ＋発火前2秒〜後3秒を素朴結合。STT は削減ラダー②で除外） |
...
家族側ブラウザで検知（RMS音圧=主トリガー＋MediaPipe 表情。STT は削減ラダー②で未接続）が
...
**注記**
- STT（発話キーワード）は削減ラダー②により未接続（`sttProvider.ts` は noop。
  `trigger_reason` は常に `rms`）。Azure Speech 取得後に実装を差し替える。
