...
| `sttProvider.ts` | STT インターフェース＋noopスタブのみ（Azure Speech 未取得＝削減ラダー②適用） |
| `index.ts` | `attachDetection({ stream, callId, onEvent })` で全配線。テスト用フック `window.__detection` を内蔵 |
...
- `trigger_reason` … `rms`（本MVPは常にRMS。STTは削減ラダー②で除外）
...
- **STT 除外**: 削減ラダー②により Azure Speech は未接続（`sttProvider.ts` は noop）。
- **検知は best-effort**: カメラ許可拒否・MediaPipe ロード失敗等でも通話は継続する。
